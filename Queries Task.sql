...
SELECT APPT_ID,date,Time,CONCAT(F_name, ' ', L_name) AS full_name,p.Phone_no, Name AS doctor_name,specialization 
FROM APPOINTMENT a
JOIN Patient p 
...
SUM(b.Total_amount) AS total_revenue, AVG(b.Total_amount) AS average_bill_amount 
...
SELECT P.Name, P.Blood_group
COUNT(a.APPT_ID) AS Total number of appointments they've had 
SUM(Total_amount)AS Total amount they've spent
FROM PATIENT 
JOIN AAPOINTMENT A ON a. APPT_ID= p. APPT_ID
JOIN billing b ON  b.Bill_ID= p .Bill_ID
WHERE payment_status ='paid',AND payment_status='pending'
GROUP BY Name,Blood_group
...
COUNT(us.usage_id) AS TimesUsed, 
SUM(quantity) AS TotalQuantityProvided, 
SUM(quantity * s.unit_price) AS TotalRevenue 
FROM Services s 
JOIN Departments d ON s.department_ID= d.department_ID
JOIN ServiceUsage us ON s.service_ID = us.service_ID
GROUP BY s.service_name, s.service_type, d.department_name
HAVING COUNT(usage_id) > 0 
...
SELECT d.docrot_ID AS DoctorID, d.doctor_name AS DoctorName, 
COUNT(a.APPT_ID) AS AppointmentCount 
FROM Doctors d 
JOIN Appointments a ON d.doctor_ID = a.doctor_ID
GROUP BY d.doctor_id, d.doctor_name 
HAVING COUNT(a.APPPT_ID) > ( SELECT COUNT(a.APPPT_ID) 
FROM APPOINTMENT a 
WHERE a.doctors_ID = 7 
...
SELECT p.Patient_ID, CONCAT(p.F_name, ' ', p.L_name) AS PatientName, p.Phone_no, 
COUNT(a.APPT_ID) AS AppointmentCount
...
SELECT b.payment_status, b.bill_id, b.amount AS TotalAmount, CONCAT(p.F_name, ' ', p.L_name) AS PatientName, dept.Dept_name AS DepartmentName 
FROM Bills b
...
INSERT INTO Appointment ( Patient_ID, Doctor_ID, Date        , Time       , Type      ,    Reason                        , Status )
